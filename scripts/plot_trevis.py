...
for row in reader:
    ...
    n = row["n"]
    if row["version"] == "blas":
        nb_threads_blas.append(int(row["nthreads"]))
        mean_blas.append(float(row["mean"]))
        stddev_blas.append(float(row["dev"]))
    else:
        nb_threads.append(int(row[""]))
...
ax.set_title(f"Temps d'exécution de l'algorithme PRR sur une matrice de taille {n}, pour le calcul de {s} valeurs propres dans un sous espace de taille {m}", wrap=True)
